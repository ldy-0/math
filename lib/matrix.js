...
/**
 * FIXME: 在不同位置使用不同方式修改值修改
 */
 Matrix.alter = function(matrix, split_arr, operate_arr, type = 'global'){
 ...
 * @return {Array}
 */
function alter(main, dash, ...callback){
 ...
 */
 function toNumber(matrix){
	 ...
	 for(let i = 0, len = matrix.length; i < len; i++){
		 matrix[i] = matrix[i] === undefined || matrix[i] === '' ? null : Number(matrix[i]);
	 }
