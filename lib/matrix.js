 ...
 */
 Matrix.average = function(matrix, type = 'global', start = 0){
	 ...
	 return dispatchOperate(type, 
									//row average handle
									function(){return matrix.map(v => average(v));},
									columnAverage,
									//global average handle
									function(){return matrix.reduce((p, v) => p+average(v), 0)/len;});
	 ...
	 function columnAverage(){
		 ...
		 matrix[0].forEach((val, i)=>{
			 ...
			 matrix.forEach(v => {
				 let cell = v[i];
				 typeof cell !== 'number' || isNaN(cell) ? leaglLength-- : arr[i] += cell ;
			 });
 ...
 *
 * @param {Array} matrix
 * @param {Array} split_arr 	start position array/不同修改方式的起始索引值集合
 * @param {Array} operate_arr alter function array/修改函数集合
 * @param {String} type 			alter pattern/修改方式
 * @return {Array}
 */
 Matrix.alter = function(matrix, split_arr, operate_arr, type = 'row'){
	 if(Array.isArray(this)){
		 [matrix, split_arr, operate_arr, type] = [this, matrix, split_arr, operate_arr];
	 }
	 
	 if(!Array.isArray(matrix)||
				!Array.isArray(split_arr)||
				!Array.isArray(operate_arr)){
		 throw new TypeError('The parameter type must be an Array');
	 }
	 
	 if(typeof matrix[0] === 'number'){
		 return toMatrix(alter(matrix, split_arr, ...operate_arr));
	 }
	 ...
	 return toMatrix(matrix.map(v => alter(v, split_arr, ...operate_arr)));
 }
 ...
 *
 * @param {Array} main 		 待修改的数组
 * @param {Array} [dash] 	 start position array/不同修改方式的起始索引值集合
 * @param {Array} operates alter function array/修改函数集合
 * @return {Array}
 */
function alter(main, dash, ...operates){
  ...
  return main.map((val, index)=>{
    for(let i = 0, len = dash.length; i<len; i++){
      ...
      if(index >= dash[i] && (i+1 === len ? true : index < dash[i+1]) ){
        return Array.isArray(val) ? val.map(v => operates[i](v)) : operates[i](val);
      }
    }
  });
 ...
 */
 function toNumber(matrix){
	 ...
	 for(let i = 0, len = matrix.length; i < len; i++){
		 let v = matrix[i];
		 matrix[i] = v === undefined || v === '' ? null : Number(v);
	 }
