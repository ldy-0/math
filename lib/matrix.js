...
/**
 *
 * Find the minimum array of specified numbers in an array/查找一维/二维数组中指定数量的最小值集合
 * 
 * @param {Array} matrix
 * @param {Number} [size=1]
 ...
 */
 Matrix.min = function(matrix, size = 1, type = 'global'){
   if(!Array.isArray(matrix) || !Number.isInteger(size))
     throw new TypeError('The parameter type is illeagl');
   
   if(!Array.isArray(matrix[0])){
     return min(matrix, size);
   }
   //FIXME: two dimensional
   return dispatchOperate(type,
                          //row
                          function(){return matrix.map(v => min(v, size))},
                          function(){},
                          function(){});
 }
...
/**
 *
 * Find the maximum array of specified numbers in an array/查找一维/二维数组中指定数量的最大值集合
 * 
 * @param {Array} matrix
 * @param {Number} [size=1]
 * @param {String} [type='global']
 ...
 */
 Matrix.max = function(matrix, size = 1, type = 'global'){
   ...
   return dispatchOperate(type, 
                          //row
                          function(){return matrix.map(v => max(v, size));},
                          function(){},
                          function(){});
 }
 ...
 */
 Matrix.add = function(matrix, addend, type = 'global'){
   ...
   return arithmetic2(matrix, addend, type, add);
 ...
 */
 Matrix.minus = function(matrix, subtractor, type='global'){
   ...
   return arithmetic2(matrix, subtractor, type, minus);
 ...
 Matrix.multiply = Matrix.times = function(matrix, multiplier, type='global'){
   ...
   return arithmetic2(matrix, multiplier, type, times);
 ...
 */
 Matrix.divide = function(matrix, divisor, type='global'){
   ...
   return arithmetic2(matrix, divisor, type, divide);
...
/**
 *
 * Find the minimum array of specified numbers in an array/查找数组中指定数量的最小值集合
 *
 * @param {Array} arr
...
/**
 *
 * Find the maximum array of specified numbers in an array/查找数组中指定数量的最大值集合
 *
 * @param {Array} arr
 * @param {Number} size 数量
 * @return {Array}
...
/**
 *
 * One/two dimensional array arithmetic/一维/二维数组算术运算
 *
 * @param {Array} matrix
 * @param {Array|Number} operand
 * @param {String} type
 * @param {Function} callback
 * @return {Array}
 */
function arithmetic2(matrix, operand, type, callback){
   if(typeof operand === 'number'){
     return Array.isArray(matrix[0]) ? matrix.map(v => callback(v, operand)) : callback(matrix, operand);
   }
   
   //matrix is one dimensional
   if(!Array.isArray(matrix[0])){
     return toMatrix(callback(matrix, operand));
   }
   
   return toMatrix(dispatchOperate(type, 
                    //row handle
                    function(){return matrix.map(v => callback(v, operand));},
                    //column and global handle
                    operate, operate));
                    
   function operate(){
     if(matrix.length !== operand.length)
       throw new RangeError('The length of the two array must be the same');
      
     return matrix.map((v, i) => callback(v, operand[i]));
   }
}
 ...
 */
 function arithmetic(main, operand, operate){
   ...
   if(typeof operand === 'number'){
     return main.map ? main.map(v => operate(v, operand)) : operate(main, operand);
   }
   
   if(Array.isArray(main) && 
      Array.isArray(operand) && 
      main.length === operand.length){
 ...
 */
 function add_minus(nub1, nub2){
   if(typeof nub1 !== 'number' && typeof nub2 !== 'number')
     throw new TypeError('The parameter must be a number');
   
   let len1 = 0, len2 = 0, m;
   
   try{len1 = nub1.toString().split('.')[1].length;}catch(e){}
   try{len2 = nub2.toString().split('.')[1].length;}catch(e){}
 ...
 */
 function multiply(nub1, nub2){
   ...
   let str1 = nub1.toString(), str2 = nub2.toString(), 
       m = 0;
 ...
 * simple convert:
 *   1. valur is undefined or null or '' -> null
 *   2. Number(value) is not NaN
 *
 * @param {Array|Number} matrix
 * @param {Integer} [precision=12]  小数点位数
 * @param {Boolean} [isSimple=true] is simple or full convert
 * @return {Array|Number}
 */
 function toNumber(matrix, precision = 12, isSimple = true){
   if(Array.isArray(this)){
     [matrix, pecision, isSimple] = [this, matrix, pecision];
   }
   ...
   for(let i = 0, len = matrix.length; i < len; i++){
     let v = matrix[i];
     //FIXME: when isSimple is true,true -> 1,false -> 0;
